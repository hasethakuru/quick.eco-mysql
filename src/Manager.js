...
class MySQL extends EventEmitter {
    ...
    constructor(options = Options) {
        ...
        /**
         * MySQL Connection
         * @type {object} mysql MySQL Connection
         */
        this.mysql;
    }
     ...
     */
    _createTable () {
        return mysql.query(`CREATE TABLE IF NOT EXISTS ${this.table} (user VARCHAR(255) UNIQUE NOT NULL, balance INT NOT NULL)`);
    ...
    write (rdata) {
        return new Promise((resolve, reject) => {
            ...
            mysql.query(`INSERT INTO \`${this.table}\` (\`user\`, \`balance\`) VALUES (?, ?);`, [ID, data],  (err, rows) => {
    ...
    read(id) {
        return new Promise((resolve, reject) => {
            ...
            mysql.query(`SELECT * FROM \`${this.table}\` WHERE \`user\` = ?`, [id], (err, res) => {
                if(err) return reject(err);
                resolve(res[0])
            })
